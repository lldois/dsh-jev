...
export function extractLatestUserPrompt(messages) {
  ...
  for (let i = messages.length - 1; i >= 0; i--) {
    ...
    if (!msg) continue;
    if (msg.source && msg.source.kind && msg.source.kind !== "user") continue;

    let text = "";
    if (typeof msg === "string") {
      text = msg.trim();
    } else if (msg.role === "user" || !msg.role) {
...
export function installAutoHook(ctx, jevClient, autoController) {
  ...
  ctx.on("agent/pre-step", async ({ agent, messages, signal }, next) => {
    ...
    const promptText = extractLatestUserPrompt(messages) || extractLatestUserPrompt(decision.messages || []);

    // Intercept slash commands sent as direct prompt text (e.g. from blank new session page or fast input)
    if (promptText && /^[\/／]?(?:jev|typesafe)(?:[:\s]+.*)?$/i.test(promptText)) {
      try {
        const rawLine = promptText.trim().replace(/^[\/／]?(?:jev|typesafe)[:\s]*/i, "");
        const isTypesafe = /^[\/／]?typesafe/i.test(promptText.trim());
        ...
        if (cmdResult) {
          ...
          const reminderMsg = createUserMessageFn({
            content: [{
              ...
              text: [
                "<system-reminder>",
                `TypeSafe Jev Command: "${promptText.trim()}"`,
