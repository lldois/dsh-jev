...
export async function getAvailableSkills(ctx, agent, signal) {
  ...
  if (ctx.skills && typeof ctx.skills.list === 'function') {
    try {
      const lookup = { cwd, signal, scope: agent };
      const list = await ctx.skills.list(lookup);
        ...
        for (const s of list) {
          if (s.name && s.description) {
            skillsMap.set(s.name, {
              ...
              location: s.resourceBase?.path || s.location,
 ...
 * @returns {Promise<{ query: string, candidates: string[], recommended: Array<{ name: string, description: string, probability: number, location?: string }>, fallbackUsed: boolean, elapsedMs: number }>}
 */
  ...
  if (jevClient && jevClient.isConfigured()) {
    try {
      ...
      const res = await jevClient.evaluate({
        state: { task: query, available_skills: candidates.map((c) => ({ name: c.name, description: c.description })) },
        questions,
      }, signal);
