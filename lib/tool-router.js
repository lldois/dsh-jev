 ...
 * @returns {Promise<{ query: string, candidates: string[], recommended: Array<{ name: string, description: string, probability: number }>, fallbackUsed: boolean, elapsedMs: number }>}
 */
  ...
  if (jevClient && jevClient.isConfigured()) {
    try {
      ...
      for (const t of candidates) {
        questions[t.name] = {
          type: 'noul',
          instructions: `Does the tool '${t.name}' (${t.description || 'no description'}) directly help accomplish this task: "${query}"?`,
        };
      }

      const res = await jevClient.evaluate({
        state: { task: query, available_tools: candidates.map((c) => ({ name: c.name, description: c.description })) },
        questions,
      }, signal);
