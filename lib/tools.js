...
export function createJevEvaluateTool(ctx, jevClient, options = {}) {
  const toolName = options.name || "jev_evaluate";
  const isEnabled = () => {
    if (typeof options.getEnabled === "function") return options.getEnabled();
    return true;
  };
  ...
  return defineTool({
    name: toolName,
    description:
      "Ask TypeSafe Jev System One typed questions (choice, noul, score) about structured state or text. Returns fast, calibrated probabilities, categories, or scores rather than generative text. Questions run in parallel and independent isolation.",
    parameters: {
      state: {
        oneOf: [
          { type: "string", description: "Context, code, or text" },
      ...
      questions: {
        ...
        required: true,
        description:
          "Map of question IDs to question definitions. Each question must specify 'type' ('choice' | 'noul' | 'score') and 'instructions'. For 'choice', specify 'criteria' mapping options to descriptions (always include 'other' or 'unclear' fallback). For 'score', specify 'criteria' as an array of rubric levels.",
      },
      model: {
        type: "string",
    ...
    output: {
      ...
      presentationMeta: (_args, value) => ({
        card: "generic",
        title: toolName === "typesafe_evaluate" ? "TypeSafe Evaluate" : "Jev Evaluate",
        kind: "decision",
    ...
    async execute(args, exec) {
      if (!isEnabled()) {
        throw new Error(
          "TypeSafe Jev is disabled. Run /typesafe enable (or set PI_TYPESAFE_ENABLED=1) to allow agent tool calls."
        );
      }
      if (!jevClient.isConfigured()) {
        throw new Error(
          "TypeSafe Jev API key is not configured. Run /typesafe login or set TYPESAFE_API_KEY environment variable."
        );
      }

      const qCount = Object.keys(args.questions || {}).length;
      if (qCount > 32) {
        return toLosslessJson(
          await jevClient.evaluateAll(
            {
              state: args.state,
    ...
    presentCall(args) {
      ...
      return {
        card: "generic",
        title: `${toolName === "typesafe_evaluate" ? "TypeSafe" : "Jev"} Evaluate (${qCount} questions)`,
        kind: "decision",
...
export function registerJevTools(ctx, jevClient, options = {}) {
  if (!ctx.tools || typeof ctx.tools.register !== "function") return [];
  const evalTool = createJevEvaluateTool(ctx, jevClient, { name: "jev_evaluate", ...options });
  ...
  // Optional typesafe_evaluate alias registration
  if (options.aliasTypesafe) {
    const typesafeEvalTool = createJevEvaluateTool(ctx, jevClient, { name: "typesafe_evaluate", ...options });
    ctx.tools.register(typesafeEvalTool);
  }
