/**
 * Tool definitions for dsh-jev:
 * - jev_evaluate: fast calibrated System One typed decisions (choice, noul, score)
 * - jev_find_skill: semantic skill discovery for specialized agent workflows
 * - jev_find_tools: semantic tool discovery across DSH registered tools
 */

import { findSkills, JEV_THRESHOLD } from './skills.js';
import { findTools } from './tool-router.js';
...
export function createJevEvaluateTool(ctx, jevClient) {
  return defineTool({
    ...
    async execute(args, exec) {
      ...
      return await jevClient.evaluate(
        {
          state: args.state,
    ...
    output: {
      ...
      render: (_args, value) => {
        ...
        if (value.recommended && value.recommended.length > 0) {
          ...
          summaryText = `Recommended tool(s) for task:\n${lines.join('\n')}`;
        } else if (value.candidates && value.candidates.length > 0) {
