...
import { registerJevCommands, isTypeSafeEnabled, setTypeSafeEnabled } from "./commands.js";
...
export function apply(ctx, config = {}) {
  ...
  if (config.enabled !== undefined) {
    setTypeSafeEnabled(config.enabled);
  }
      ...
      try {
        ...
        if (z && typeof z.object === "function") {
          const ConfigSchema = z.object({
            ...
            enabled: z.boolean().default(true).description("Enable TypeSafe evaluation tool calls"),
          ...
          settingsCtx.settings.installSection(ctx, SETTINGS_NAMESPACE, ConfigSchema, config, {
            setSource: (source) => {
              ...
              if (source && typeof source().enabled === "boolean") {
                setTypeSafeEnabled(source().enabled);
              }
            },
            onChange: () => {},
          });
  ...
  // 1. Register tools (jev_evaluate, typesafe_evaluate, jev_find_skill, jev_find_tools)
  registerJevTools(ctx, jevClient, {
    getEnabled: () => isTypeSafeEnabled(),
  });

  // 2. Register slash commands (/typesafe and /jev) dynamically when commands service is ready
  if (ctx.inject) {
