  ...
  return ctx.commands.register({
    ...
    handler: async (invocation) => {
      ...
      if (sub === 'status' || sub === '') {
        ...
        const availableTools = getAvailableTools(ctx, invocation.agent);
        const availableSkills = await getAvailableSkills(ctx, invocation.agent, invocation.signal).catch(() => []);

        const statusLines = [
          '=== TypeSafe Jev System One Status ===',
          `• Configured: ${origin ? `Yes (from ${origin})` : 'No (set TYPESAFE_API_KEY)'}`,
          `• Default Model: ${jevClient.defaultModel}`,
          `• API Endpoint: ${jevClient.baseURL}`,
          `• Session Requests: ${jevClient.stats.requestsCount}`,
          `• Total Tokens: ${jevClient.stats.totalTokens} (outputs are free - Jev does not generate text)`,
          `• Last Latency: ${jevClient.stats.lastElapsedMs}ms`,
      ...
      if (sub === 'test' || sub === 'eval' || sub === 'evaluate') {
        ...
        try {
          const res = await jevClient.evaluate(request, invocation.signal);
          const lines = [
      ...
      if (sub === 'skills' || sub === 'skill') {
        const query = rest;
        if (!query) {
          const available = await getAvailableSkills(ctx, invocation.agent, invocation.signal).catch(() => []);
      ...
      if (sub === 'tools' || sub === 'tool') {
        const query = rest;
        if (!query) {
          const available = getAvailableTools(ctx, invocation.agent);
          if (available.length === 0) {
